...
\title{%
    ...
    \large NM i AI 2026 --- 1st Place Solution (0.7113 mAP)
}
...
We present our solution to the NorgesGruppen grocery shelf product detection task (Task~3), which achieved 0.7113~mAP on the private test set.
...
The core of our submission is a heterogeneous ensemble of three architectures, each contributing a different ``view'' of the data.

\begin{table}[H]
\centering
\caption{The three models in the final ensemble.}
\label{tab:models}
\begin{tabular}{@{}llll@{}}
\toprule
...
The path to our best result runs through architecture diversity and multi-scale inference, while post-processing and specialization directions consistently showed diminishing or negative returns.
...
\begin{scope}[shift={(-10,-9.5)}]
  ...
  \node[best, anchor=west] at (0, 0.6) {In final submission};
...
\end{tikzpicture}
\caption{Technique exploration radar showing all approaches tried during the competition. The path to our best result runs through architecture diversity (North) and multi-scale inference (ENE). Post-processing and specialization (red nodes) consistently failed to generalize.}
\label{fig:radar}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Competition submissions with public test scores. The final submission achieved 0.9226 on public and 0.7113 on private test.}
...
Post-competition discussion in the official Slack channel revealed that many teams explored similar directions, arriving at converging conclusions.
This section summarizes shared findings and open questions from the community.

\subsection{Synthetic Data}

Synthetic data generation was one of the most popular strategies, with at least six teams reporting results.
Andr\'e Mikalsen (0.920) used Nano Banana~2 to generate product image variations in shelf-realistic contexts (different angles, shadows, lying down).
Andreas Hatlem (0.9220) generated approximately 1,400 synthetic images by compositing reference photos onto shelf backgrounds.
Markus Hei physically visited stores to photograph underrepresented products and used Gemini with SAM3 for silver labeling.
Dag Koding from DKIT experimented with synthetic data and even considered 3D models of product packaging.
Ludvig {\O}vrevik produced 3,000 synthetic images using Nano Banana.
J{\o}rgen R{\o}rvik generated 50 images per product class using Nano Banana with cropped references.

The consistent finding across all teams was that synthetic data did not improve scores.
We did not attempt synthetic data ourselves, instead investing time in architecture diversity.
In retrospect, we believe the domain gap between studio product photos and real shelf crops is the primary obstacle --- compositing techniques cannot replicate the complex interactions of lighting, occlusion, price tags, and shelf geometry that characterize real retail images.
Additionally, synthetic data risks reducing ensemble diversity if all models train on the same generated artifacts.

\subsection{Converging Findings}

Several findings were independently confirmed by multiple teams.
Training on all 248 images (without a validation split) was reported as a significant improvement by Hatlem (+0.004) and matches our experience.
Mikalsen worked around the lack of validation data by generating synthetic validation images with Gemini~3.1 Flash for classification labels.
Multi-model WBF ensembles were the dominant strategy among top teams.
Hatlem independently discovered the same diversity principle we relied on: his weakest individual model (K-fold~2, 0.749~mAP) consistently gave the best ensemble results, and swapping it for stronger models always hurt the ensemble score.
Walgermo from Matriks (0.9208) also used a 3-model ensemble with random seed diversity.
Post-processing tuning (WBF parameters, Soft-NMS, temperature scaling) was reported as unhelpful by both Hatlem and ourselves.

Eirik B{\ae}kkelund took a different approach with DINOv2 ViT-B/14 and an ArcFace head for classification, achieving roughly 94\% accuracy on reference embeddings.
His main issue was coffee brand confusion (Ali Kaffe alone accounted for 20\% of errors), and he noted that lightweight OCR models could potentially resolve these confusions within the 420\,MB file size budget.

\subsection{Open Questions}

\paragraph{The unknown product problem.}
P{\aa}l Bentsen raised an interesting question about the task specification: should products present in the image but not belonging to any of the 356 annotated categories be detected and classified as ``unknown,'' or ignored entirely?
As shown in Figure~\ref{fig:unknown}, the Natreen sweetener products on the shelf have no ground truth annotations despite being clearly visible.
In a real-world retail system, detecting all products would be essential for inventory management.
The competition's handling of this edge case was ambiguous, and the answer directly affects model design --- detecting products that the evaluator ignores generates false positives that hurt the score.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{memes/unknown_product.png}
\caption{The unknown product problem, raised by P{\aa}l Bentsen.
The Natreen products on the lower shelf have no ground truth annotations despite being clearly visible.
In a real-world system these should be detected; in the competition, detecting them may hurt the score.}
\label{fig:unknown}
\end{figure}

\paragraph{Annotation quality.}
Bentsen also observed cases where the ground truth contained errors --- correct model predictions that lacked corresponding annotations.
They manually corrected a selection and retrained, an approach that highlights the tension between trusting provided labels and investing time in data quality.
...
Our most important lesson is that optimizing for a known test set is risky when the final evaluation uses a different distribution.
Our public leaderboard score (0.9226, 4th place) did not predict our private test result (0.7113), and the gap between public and private rankings was significant across teams.
Designing for robustness rather than maximizing a single metric proved more reliable.

\textbf{Diversity over specialization.}
In our experiments, every attempt at specialization (classification loss tuning, two-stage classifiers, gallery re-ranking, model soups) failed to generalize.
Every form of diversity (architecture, augmentation, scale) helped.
On small datasets, the lesson is to invest in making \emph{different types of predictions} rather than \emph{better predictions of the same type}.

\textbf{Reject locally-good changes.}
The hardest discipline was rejecting techniques that showed clear local improvements.
Our \texttt{cls=2.0} model scored +0.015 higher locally --- a massive improvement by any measure.
But it showed zero test improvement.
Learning to distinguish genuine gains from overfitting artifacts, without a reliable validation set, was one of the hardest challenges of the competition.
...
We presented our solution to the NM i AI 2026 NorgesGruppen product detection task, achieving 0.7113~mAP on the private test set.
